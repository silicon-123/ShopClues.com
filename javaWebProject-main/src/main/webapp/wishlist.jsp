...
<%
    ...
    try {
        ...
        String query = "SELECT p.productid, p.price, p.imageurl, c.quantity FROM product p INNER JOIN wishlist c ON p.productid = c.productid WHERE c.customerid = ?";
...
%>
            <div style="border:solid 1px black;border-radius:10px; margin:50px;padding:30px">
               ...
               <button onclick="redirectToOrder(<%= productId %>)" style="padding: 15px; background-color: #FBAB7E; background-image: linear-gradient(62deg, #FBAB7E 0%, #F7CE68 100%); margin:10px; border-radius:10px; width:20% ;margin-left:300px">Add to cart</button>

<script>
function redirectToOrder(productId) {
    // Set productId in session-like storage
